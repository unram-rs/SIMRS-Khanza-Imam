    ...
    private void tampil() {
        ...
        try{
            ps=koneksi.prepareStatement(
               ...
               "(" +
               "IFNULL((SELECT SUM(trf_kamar*lama) FROM kamar_inap ki WHERE ki.no_rawat=reg_periksa.no_rawat),0)+ " +
               "IFNULL((SELECT SUM(biaya_reg) FROM reg_periksa rg WHERE rg.no_rawat=reg_periksa.no_rawat),0)+ " +        
               "IFNULL((SELECT SUM(biaya_rawat) FROM rawat_inap_drpr rid WHERE rid.no_rawat=reg_periksa.no_rawat),0)+ " +
               "IFNULL((SELECT SUM(hargasatuan*jumlah) AS biaya FROM beri_obat_operasi boo WHERE boo.no_rawat=reg_periksa.no_rawat),0)+ " +
               "IFNULL((SELECT SUM(biaya_dokter_pjanak+biayaperawat_luar+biayabidan+biayadokter_anak+biayadokter_anestesi+biayaasisten_operator1+biayaoperator1+biayasarpras+biayasewaok+akomodasi+bagian_rs) AS biaya FROM operasi o WHERE o.no_rawat=reg_periksa.no_rawat),0)+ " +        
               "IFNULL((SELECT SUM(biaya_rawat) FROM rawat_jl_drpr rjd WHERE rjd.no_rawat=reg_periksa.no_rawat),0)+ " +
               "IFNULL((SELECT SUM(biaya_obat*jml) FROM detail_pemberian_obat dpo WHERE dpo.no_rawat=reg_periksa.no_rawat),0)+ " +
               "IFNULL((SELECT SUM(biaya) FROM periksa_lab pl WHERE pl.no_rawat=reg_periksa.no_rawat),0)+ " +
               "IFNULL((SELECT SUM(biaya) FROM periksa_radiologi pr WHERE pr.no_rawat=reg_periksa.no_rawat),0) " +
               ") AS ttl_piu," +
               "kamar_inap.stts_pulang,kamar_inap.ttl_biaya,kamar_inap.lama,dokter.nm_dokter,kamar_inap.kd_kamar,reg_periksa.kd_pj,concat(reg_periksa.umurdaftar,' ',reg_periksa.sttsumur)as umur,reg_periksa.status_bayar, "+
            ...
            try {
                ...
                while(rs.next()){
                    tabMode.addRow(new String[]{
                        ...
                        rs.getString("kamar"),Valid.SetAngka(rs.getDouble("trf_kamar")) + " (" + Valid.SetAngka(rs.getDouble("ttl_piu")) + ")",rs.getString("diagnosa_awal"),
